"""
Code for training Pix2Pix model using discriminator model parallelism.
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for x_, _ in train_dataloader:
            ...
            x_, y_ = Variable(x_.cuda('cuda:0')), Variable(y_.cuda('cuda:0'))
            
            D_result_real = D(x_,y_).squeeze()
            D_real_loss = BCE_loss(D_result_real, Variable(torch.ones(D_result_real.size()).cuda('cuda:1')))

            G_result = G(x_).detach()
            D_result_fake = D(x_, G_result).squeeze()
            ...
            G_train_loss = BCE_loss(D_result.cuda('cuda:0'), Variable(torch.ones(D_result.size()).cuda('cuda:0'))) + L1_lambda * L1_loss(G_result, y_)

...
class discriminator(nn.Module):
    ...
    def __init__(self, d=64):
        ...
        self.seq1 = nn.Sequential(nn.Conv2d(6, d, 4, 2, 1),
                                    nn.leaky_relu(0.2),
                                    nn.Conv2d(d, d * 2, 4, 2, 1),
                                    nn.BatchNorm2d(d * 2),
                                    nn.leaky_relu(0.2)).to('cuda:0')
         
        self.seq2 = nn.Sequential(nn.Conv2d(d * 2, d * 4, 4, 2, 1),
                                    nn.BatchNorm2d(d * 4),
                                    nn.leaky_relu(0.2),
                                    nn.Conv2d(d * 4, d * 8, 4, 1, 1),
                                    nn.BatchNorm2d(d * 8),
                                    nn.leaky_relu(0.2),
                                    nn.Conv2d(d * 8, 1, 4, 1, 1)).to('cuda:1')
    ...
    def forward(self, input, label):
        ...
        return torch.cat(ret) #self.conv5(x)

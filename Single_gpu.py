"""
Code for training Pix2Pix baseline model.
...
def main():
    ...
    for epoch in range( NUM_EPOCHS):
        ...
        for x_, _ in train_dataloader:
            ...
            D_result_real = D(Variable(x_.cuda()), Variable(y_.cuda())).squeeze()
            D_real_loss = BCE_loss(D_result_real, Variable(torch.ones(D_result_real.size()).cuda()))

            G_result = G(Variable(x_.cuda())).detach().cuda()
            D_result_fake = D(Variable(x_.cuda()), G_result).squeeze()
            D_fake_loss = BCE_loss(D_result_fake, Variable(torch.zeros(D_result_fake.size()).cuda()))
            ...
            G_result = G(Variable(x_.cuda()))
            D_result = D(Variable(x_.cuda()), G_result).squeeze()

            G_train_loss = BCE_loss(D_result, Variable(torch.ones(D_result.size()).cuda())) +  L1_lambda * L1_loss(G_result.cuda(), Variable(y_.cuda()))

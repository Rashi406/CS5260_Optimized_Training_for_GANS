...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for x_, _ in train_dataloader:
            ...
            G_result = G(Variable(x_.cuda('cuda:0'))).detach()
            ...
            G_result = G(Variable(x_.cuda('cuda:0')))
            D_result = D(Variable(x_.cuda('cuda:1')), G_result).squeeze()

            G_train_loss = BCE_loss(D_result, Variable(torch.ones(D_result.size()).cuda('cuda:1'))) + L1_lambda * L1_loss(G_result, Variable(y_.cuda('cuda:1')))
